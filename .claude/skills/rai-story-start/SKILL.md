...
- **Skipping `/rai-story-start` and working directly on `main`/`dev`**: The most critical process violation is beginning story work (design, plan, implementation) without first running `/rai-story-start` to create the dedicated `story/s{N}.{M}/{slug}` branch from `{dev_branch}`. This happened in this session — S5.4 design, plan, and T1 prompt template were all created on `main` before the user caught it. The fix required: (1) creating the branch retroactively, (2) updating `Owner:` and `Branch:` fields in `story.md`, (3) committing all artifacts on the story branch. **Always run `/rai-story-start` as the very first step for any new story — no exceptions.** The branch is not optional; it isolates the story's commits, enables clean merge/deletion after close, and is required by the RaiSE workflow.
